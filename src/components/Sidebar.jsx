...
import { Users, Settings, Package, FileText, Bell, Shield, Image, Tag, Menu, X } from 'lucide-react';
...
const menuItems = [
  ...
  { id: 'offers', label: 'Khuyến mãi', icon: Tag, path: '/offers' },
  { id: 'service-order-images', label: 'Hình ảnh đơn', icon: Image, path: '/service-order-images' },
];
...
export default function Sidebar() {
  ...
  console.log('Sidebar rendered, currentPath:', currentPath);

  const handleClick = (item) => {
    setActiveItem(item.id);
    navigate(item.path);
    if (window.innerWidth < 1024) setIsOpen(false);
    console.log('Sidebar nav clicked:', item.label, '->', item.path);
  };
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={`fixed z-50 top-0 left-0 h-dvh bg-gray-800 text-white transform transition-transform duration-300 ease-in-out lg:static lg:translate-x-0 ${
        isOpen ? 'translate-x-0' : '-translate-x-full'
      } w-56 sm:w-64 lg:w-64 xl:w-72 2xl:w-80 overflow-y-auto shadow-lg`}>
        <div className="flex items-center justify-between p-3 sm:p-4 border-b border-gray-700">
          <div className="text-lg sm:text-xl font-bold truncate">Admin Dashboard</div>
